  ...
  public renderDayNodes() {
    ...
    dayNodes.push(
      <span key="prev-month" className={cx('other-month-ranger')}>
        {Array.from(Array(firstDay).keys())
          .reverse()
          .map(i => (
            <CalendarDate
              key={`prev-${i}`}
    ...
    for (let i = firstDate; i <= lastDate; i++) {
      ...
      if (inRange) {
        ...
        for (let j = rangeStart; j <= rangeEnd; j++) {
          nodes.push(
            <CalendarDate key={`now-${j}`} onClick={() => onClick(year, month, j)} isToday={j === today}>
              {j}
            </CalendarDate>
          );
        }
    ...
    return (
      <div className={cx('days-header')}>
        {Array.from(weekNameMap.values()).map(text => (
          <span key={text} className={cx('cube')}>
            {text}
          </span>
        ))}
      </div>
  ...
  public render() {
    return (
      <div className={cx('body')}>
        ...
        <div className={cx('days-body')}>{this.renderDayNodes()}</div>
      </div>
    );
  }
}
